...
export default function DefenseHero({ onOpenInquiry }: DefenseHeroProps) {
  return (
    <section
      id="defense-hero"
      className="relative min-h-0 sm:min-h-[85vh] lg:min-h-[90vh] flex flex-col justify-between bg-[#020713] overflow-hidden pb-6 sm:pb-10 lg:pb-12 border-b border-white/[0.08]"
    >
      {/* 1. TOP NAVBAR */}
      <Navbar
        currentRoute="/defense"
        onSelectCategory={() => onOpenInquiry?.("Defense")}
      />

      {/* 2. CINEMATIC OCEAN BACKGROUND */}
      <div className="absolute inset-0 z-0 overflow-hidden pointer-events-none select-none">
        <video
          ...
          playsInline
          preload="auto"
          className="w-full h-full object-cover opacity-60 filter contrast-110 saturate-110 brightness-95"
        >
          <source src="/Water-landscape.mp4" type="video/mp4" media="(min-width: 768px)" />
          <source src="/Water.mp4" type="video/mp4" />
        </video>

        {/* Soft, minimal cinematic ocean gradients */}
        <div className="absolute inset-0 bg-gradient-to-t from-[#020713] via-black/40 to-black/60 pointer-events-none" />
      ...
      {/* 3. HERO LEFT-ALIGNED CONTENT (MATCHING HOME PAGE HERO SECTION) */}
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-8 md:px-12 w-full flex-1 flex flex-col items-start justify-center text-left pt-8 sm:pt-12 lg:pt-16 pb-4 sm:pb-6 lg:pb-8">
        {/* Monumental Headline */}
        <h1
        ...
        <p className="mt-2.5 min-[380px]:mt-3.5 sm:mt-6 text-xs sm:text-base md:text-lg text-zinc-300 max-w-2xl font-light leading-relaxed">
          Autonomous monolithic titanium submersibles and multi-vehicle autonomous swarms designed for contested, GPS-denied depths. Silent by physics. Decisive by design.
        </p>
        ...
        <div className="mt-4 min-[380px]:mt-5 sm:mt-8 flex flex-row items-center gap-2 sm:gap-4 max-w-full overflow-x-visible">
          <Button
            variant="primary"
            shape="pill"
            leftIcon={<Shield className="w-3 h-3 sm:w-3.5 sm:h-3.5 shrink-0" />}
          ...
          >
            <span className="hidden min-[440px]:inline">Request Defense Brief</span>
            <span className="min-[440px]:hidden">Defense Brief</span>
          </Button>
